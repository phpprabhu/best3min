...
if [[ "$current_day" -le 5 && "$current_time" > "09:30" && "$current_time" < "15:30" ]]; then
    cd /root/best3min/ && export FLASK_APP=app.py && flask check_entry >> /root/best3min/log/check_entry.log 2>&1
fi